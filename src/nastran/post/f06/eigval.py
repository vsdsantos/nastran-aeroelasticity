...
class RealEigValF06Page(F06Page):
    ...
    def __repr__(self):
        return f"REAL EIGVAL F06\tSUBCASE {self.info['SUBCASE']}\tPAGE {self.meta['page']}"
...
def summarize_real_eigvals(results, key="CYCLES"):
    vals = [(p.info["SUBCASE"], p.df[key]) for p in results.eigval]
...
class ModalEffectiveMassFractionF06Page(F06Page):
    ...
    def is_page_of_this_type(cls, lines, previous_page_type):
        ...
        return (
            len(lines) - 1 >= MODALMASSFRAC_LINE and MODALMASSFRAC_STR in lines[MODALMASSFRAC_LINE]
        )
