...
from nastran.post.f06.common import (
    F06Page,
    extract_tabulated_data,
    find_tabular_line_range,
    parse_label_subcase,
    parse_text_value,
)
...
p_info = {key: re.compile(rf"\b{key} =\s*\S*") for key in FLUTTER_INFO_KEYS}
...
class FlutterF06Page(F06Page):
    ...
    def __repr__(self):
        return f"FLUTTER F06 PAGE\tSUBCASE={self.info['SUBCASE']}\tLABEL={self.info['LABEL']}\tMODE={self.info['POINT']}"
...
def get_critical_roots(df, epsilon=1e-9, var_ref="DAMPING"):
    ...
    critic_idx = (
        df.loc[df[var_ref] >= -epsilon, "VELOCITY"].groupby(indexes).apply(lambda df: df.idxmin())
    )
...
def _parse_summary_info(lines):
    ...
    for key in FLUTTER_INFO_KEYS:
        p = p_info[key]
        value = p.search(raw).group(0).replace(f"{key} =", "").strip()
...
def _is_continuation(i, pages):
    if i > 0 and pages[i - 1].df is not None:
        last_info = (
            pages[i - 1].info["SUBCASE"],
            pages[i - 1].info["MACH NUMBER"],
            pages[i - 1].info["POINT"],
        )
        return last_info == (
            pages[i].info["SUBCASE"],
